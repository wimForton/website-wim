...
export class CurveSegment{
    ...
    constructor(scene: THREE.Scene, splineHelperObjects: THREE.Mesh[], v0: THREE.Vector3, v1: THREE.Vector3, v2: THREE.Vector3, v3: THREE.Vector3, controlscale: number[] = [1,1,1]) {
        ...
        const color1 = new THREE.Color().setRGB( 1, 1, 0 );
        const material1 = new THREE.MeshBasicMaterial( { color: color1 } );
        const color2 = new THREE.Color().setRGB( 0, 1, 1 );
        const material2 = new THREE.MeshBasicMaterial( { color: color2 } );
        const color3 = new THREE.Color().setRGB( 0, 1, 1 );
        const material3 = new THREE.MeshBasicMaterial( { color: color3 } );
        const color4 = new THREE.Color().setRGB( 1, 1, 0 );
        const material4 = new THREE.MeshBasicMaterial( { color: color4 } );
